...
{
	Reg(get_GPIO_port(lcd_def->Data_GPIO_port) + GPIO_ODR) = cmd;
...
{
  Reg(get_GPIO_port(lcd_def->CMD_GPIO_port)+GPIO_ODR) = 0;
...
void writeCommand(LCDdef* lcd_def, unsigned short cmd)
{
  ...
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_RS, 0);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_RW, 0);
  delay_ms(10);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_EN, 0);
  writeByte(lcd_def, cmd);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_EN, 1);
  delay_ms(10);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_EN, 0);
}
...
{
  Reg(get_GPIO_port(lcd_def->Data_GPIO_port)+GPIO_ODR) = 0;
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_RS, 1);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_RW, 0);
  delay_ms(100);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_EN, 0);
  GPIO_Write_Output(lcd_def->CMD_GPIO_port, lcd_def->PIN_EN, 1);
...
void displayStr(LCDdef* lcd_def, char* str)
{
  // set DDRAM addr
